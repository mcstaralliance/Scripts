...
ItemEvents.tooltip((tooltip) => {
    ...
    tooltip.add(
        'kubejs:custom_chest',
        '可到点券商城购买奇珍之匙开箱，放置到地上使用奇珍之匙右键即可'
    );
    tooltip.add(
        'kubejs:custom_chest',
        '内含：加速火把，派蒙矿机，无尽装备，MekaSuit装备等稀有物品'
    );
    ...
    tooltip.add(
        'starcraft:simple_miner',
        '可消耗石头、电量与魔力随机挖掘出各种矿石，顺带一提这玩意其实有 27 个输出格'
    );
    tooltip.add('starcraft:simple_miner', [
        Text.of('控制器方块换成纯净玻璃，用红石右键成型').red()
    ]);
    tooltip.add(
        'enderio:powered_spawner',
        '你需要找到一个破损的刷怪笼按u键以查看他的配方'
    );
    ...
    tooltip.addAdvanced(Ingredient.all, (item, advanced, text) => {
        if (!advanced) {
            let tags = item.tags.toArray();
            if (tags.length > 0) {
                if (!tooltip.ctrl) {
                    text.add(Text.gray('按住 Ctrl 显示标签'));
                } else {
                    tags.forEach((tag) => {
                        text.add(
                            Text.gray('#').append(tag.location().toString())
                        );
                    });
                }
            }
        ...
        if (item.nbt) {
            if (!tooltip.shift) {
                text.add(Text.gray('按住 Shift 显示 NBT'));
            } else {
                text.add(Text.prettyPrintNbt(item.nbt));
            }
        }
    });
});
